...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False):
        ...
        self.image_adapter = None   # used for lora image2image generation
        self.image_adapter_img_in = None   # used for lora image2image generation
    ...
    def set_lora(self, local_path: str = None, repo_id: str = None,
                 ...
                 image_adapter_path: str = None, image_adapter_img_in_path: str = None,):
        checkpoint = load_checkpoint(local_path, repo_id, name)
        self.update_model_with_lora(checkpoint, lora_weight)

        if image_adapter_img_in_path is not None:   # use image2image adapter
            self.image_adapter = ImageAdapter().to(self.device)
            self.image_adapter_img_in = ImageAdapter_img_in().to(self.device)
            image_adapter_state = load_checkpoint(image_adapter_path, repo_id, name)    # .safetensors
            image_adapter_img_in_state = load_checkpoint(image_adapter_img_in_path, repo_id, name)  # .safetensors
            self.image_adapter.load_state_dict(image_adapter_state)
            self.image_adapter_img_in.load_state_dict(image_adapter_img_in_state)
    ...
    def __call__(self,
        ...
        if self.controlnet_loaded or self.image_adapter_img_in is not None: # controlnet or image2image adapter
            if self.controlnet_loaded:  # only use annotator for controlnet
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if self.controlnet_loaded:
                ...
                if self.image_adapter_img_in is not None:   # image2image adapter
                    x_cond = self.ae.encode(controlnet_image.to(self.device).float())
                    x_cond = self.image_adapter(x_cond)
                    x_cond = rearrange(x_cond, "b c (h ph) (w pw) -> b (h w) (c ph pw)", ph=2, pw=2)
                    x_cond = self.image_adapter_img_in(x_cond).to(x.dtype)
                else:
                    x_cond = None
                x = denoise(
                    self.model,
...
class XFluxPipeline:
    ...
    def forward(
        ...
        with torch.no_grad():
            if self.offload:
                self.t5, self.clip = self.t5.to(self.device), self.clip.to(self.device)
            inp_cond = prepare(t5=self.t5, clip=self.clip, img=x, prompt=prompt, global_prompt=global_prompt)
            neg_inp_cond = prepare(t5=self.t5, clip=self.clip, img=x, prompt=neg_prompt)
